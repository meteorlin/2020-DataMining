...
def get_link_web_portrait(concat_result_dir: str, link_attr_path: str, link_topo_path: str,
                          slice_id_list: list, output_dir: str = './link_web_portrait_result'):
    """
    ...
    * 程序流程：
    ...
    :param slice_id_list: 需要输出的slice_id的列表，如[261, 271]，代表需要输出261和271两个slice_id下的路网子系统道路画像
    :param output_dir: 输出路径，默认是在当前目录下的'link_web_portrait_result'文件夹下输出
    :return: 输出所需slice_id的csv文件，命名方式为“{slice_id}_train_data.csv”
    """
    # 读取相关的属性文件
    ...
    bar = tqdm(total=len(concat_result_list) * len(slice_id_list))
    for file_name in concat_result_list:
        ...
        for slice_id in slice_id_list:
            concat_data_point_slice_id = concat_data[concat_data['slice_id'] == slice_id]
            ...
            for _, concat_row in concat_data_point_slice_id.iterrows():
                ...
                for _, link_relationship in current_link_topo.iterrows():
                    ...
                    if not len(truth_link_df) == 0:
                        ...
                        single_truth_link_dict.setdefault('predict_link_id', truth_link_df.iloc[0].at['link'])
            ...
            if not os.path.exists(output_dir):
                os.mkdir(output_dir)
            pointed_slice_id_df.to_csv(os.path.join(output_dir, str(slice_id) + '_web_portrait.csv'),
